...
def run_time_based(
    ...
    baseline_time = _measure_baseline(injector, url, method, params, param)
...
def _timed_fetch(
    ...
    """Send request with *value* injected and return elapsed seconds."""
    t0 = time.monotonic()
    try:
        if method.upper() == "POST":
            injector.post(url, data={**params, param: value})
        else:
            injector.inject_get(url, param, value)
